...
//    int mDebugDepth = 0;
     ...
     * @param mNodeInfo
     */
    private void textTree(AccessibilityNodeInfo mNodeInfo, List<AccessibilityNodeInfo> out, int SCREEN_W, int SCREEN_H){
        if (mNodeInfo == null) return;

        for (int i = 0; i < mNodeInfo.getChildCount(); i++) {
            textTree(mNodeInfo.getChild(i), out, SCREEN_W, SCREEN_H);
        }
...
//        }
        ...
        if (txt != null && txt.length() > 0
            ...
            Rect r = new Rect();
            mNodeInfo.getBoundsInScreen(r);
            // reject empty or fully-offscreen rects
            if (!r.isEmpty()
                    && r.left <  SCREEN_W
                    && r.top  <  SCREEN_H
                    && r.right > 0
    ...
    private boolean shouldTrack(String curr_app) {
        int criteria = Integer.parseInt(
                Aware.getSetting(getApplicationContext(), Aware_Preferences.PACKAGE_SPECIFICATION)
        );
        String appNames = Aware.getSetting(getApplicationContext(), Aware_Preferences.PACKAGE_NAMES);

        if (criteria == 0) {               // inclusive only
            return appNames.contains(curr_app);
        } else if (criteria == 1) {        // exclusive only
            return !appNames.contains(curr_app);
        }
        return true;                       // criteria == 2, track all
    }
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        ...
        final int SCREEN_W = dm.widthPixels;
        final int SCREEN_H = dm.heightPixels;

        handleScreenText(event, SCREEN_W, SCREEN_H);
        handleNotifications(event);
        handleForegroundApps(event);
        handleKeyboard(event);
        handleTouch(event);
    }

    private void handleScreenText(AccessibilityEvent event, int SCREEN_W, int SCREEN_H) {
        if (Aware.getSetting(getApplicationContext(), Aware_Preferences.STATUS_SCREENTEXT).equals("true") && getScreenStatus() == 0) {
            long currentTime = System.currentTimeMillis();
            if (currentTime - lastCaptureTime < 500) return; // Throttle to 500ms
            Log.d(TAG, "PREV TIME: " + lastCaptureTime);
            ...
            textTree(root, textNodes, SCREEN_W, SCREEN_H);
            ...
            for (AccessibilityNodeInfo node : textNodes) {
                ...
                try {
                    ...
                    if (node.getViewIdResourceName() != null) {
                        payload.put("resourceId", node.getViewIdResourceName());
                    }

                    if (node.getContentDescription() != null) {
                        payload.put("contentDesc", node.getContentDescription().toString());
                    }

                    allNodes.put(payload);
    ...
    private void handleKeyboard(AccessibilityEvent event) {
    ...
    private void handleTouch(AccessibilityEvent event) {
